...
validate-winget-manifests:
	pwsh -Command "$$manifestDir = './packaging/winget/manifests/u/UlrichZogo/OCGO/0.1.0'; if (-not (Test-Path (Join-Path $$manifestDir 'UlrichZogo.OCGO.yaml'))) { throw 'Missing OCGO.yaml' }; if (-not (Test-Path (Join-Path $$manifestDir 'UlrichZogo.OCGO.installer.yaml'))) { throw 'Missing installer.yaml' }; if (-not (Test-Path (Join-Path $$manifestDir 'UlrichZogo.OCGO.locale.en-US.yaml'))) { throw 'Missing locale.yaml' }; if (Get-Command winget -ErrorAction SilentlyContinue) { winget validate $$manifestDir; if ($$LASTEXITCODE -ne 0) { exit $$LASTEXITCODE }; Write-Host 'winget validate completed.' } else { Write-Host 'winget not available; skipping.' }"
