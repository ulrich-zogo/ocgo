...
validate-winget-manifests:
	pwsh -Command "$$manifestDir = './packaging/winget/manifests/u/UlrichZogo/OCGO/0.1.0'; if (-not (Test-Path (Join-Path $$manifestDir 'UlrichZogo.OCGO.yaml'))) { throw 'Missing OCGO.yaml' }; if (-not (Test-Path (Join-Path $$manifestDir 'UlrichZogo.OCGO.installer.yaml'))) { throw 'Missing installer.yaml' }; if (-not (Test-Path (Join-Path $$manifestDir 'UlrichZogo.OCGO.locale.en-US.yaml'))) { throw 'Missing locale.yaml' }; if (Get-Command winget -ErrorAction SilentlyContinue) { $$out = winget validate $$manifestDir 2>&1; $$out | Write-Host; $$ec = $$LASTEXITCODE; if ($$ec -ne 0) { if ($$out -match 'succeeded') { Write-Host 'winget validate succeeded (with warnings).' } else { exit $$ec } } else { Write-Host 'winget validate completed.' } } else { Write-Host 'winget not available; skipping.' }"
