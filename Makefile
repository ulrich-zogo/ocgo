...
validate-winget-manifests:
	pwsh -Command "$$root = './packaging/winget/manifests/u/UlrichZogo/OCGO/0.1.0'; $$checks = @{ 'UlrichZogo.OCGO.yaml' = @('PackageIdentifier: UlrichZogo.OCGO', 'ManifestType: version'); 'UlrichZogo.OCGO.installer.yaml' = @('PackageIdentifier: UlrichZogo.OCGO', 'InstallerType: zip'); 'UlrichZogo.OCGO.locale.en-US.yaml' = @('PackageIdentifier: UlrichZogo.OCGO', 'PackageLocale: en-US') }; $$ok = $$true; foreach ($$f in $$checks.Keys) { $$path = Join-Path $$root $$f; if (-not (Test-Path $$path)) { Write-Error \"Missing: $$f\"; $$ok = $$false; continue }; $$content = Get-Content $$path -Raw; foreach ($$key in $$checks[$$f]) { if ($$content -notmatch [regex]::Escape($$key)) { Write-Error \"$$f missing: $$key\"; $$ok = $$false } }; if ($$ok) { Write-Host \"$$f: valid\" } }; if (-not $$ok) { exit 1 }"
